...
const formatStylish = (data1, data2) => {
  const iter = (obj1, obj2, depth = 1) => {
    ...
    const lines = keys.flatMap((key) => {
      ...
      if (has1 && !has2) {
        return `${signIndent}- ${key}: ${formatValue(obj1[key], depth + 1)}`;
      }

      if (!has1 && has2) {
        return `${signIndent}+ ${key}: ${formatValue(obj2[key], depth + 1)}`;
      }
      ...
      if (obj1[key] === obj2[key]) {
        return `${currentIndent}${key}: ${formatValue(obj1[key], depth + 1)}`;
      }

      return [
        `${signIndent}- ${key}: ${formatValue(obj1[key], depth + 1)}`,
        `${signIndent}+ ${key}: ${formatValue(obj2[key], depth + 1)}`,
      ];
    });
